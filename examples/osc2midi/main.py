...
from rtmidi.midiconstants import (BANK_SELECT_LSB, BANK_SELECT_MSB, CHANNEL_PRESSURE,
                                  CHANNEL_VOLUME, CONTROLLER_CHANGE, NOTE_OFF, NOTE_ON, PITCH_BEND,
                                  POLYPHONIC_PRESSURE, PROGRAM_CHANGE, SONG_START, SONG_STOP)
...
class OSC2MIDIHandler(object):
    ...
    def sendcc(self, value, cc=0, channel=None, invert=False, **kwargs):
        ...
        self.midiout.send(MidiEvent.fromdata(CONTROLLER_CHANGE, channel=channel,
                                             data=[cc & 0x7f, value]))

    def sendtwocc(self, val1, val2, cc1=0, cc2=32, channel=None, invert=False, **kwargs):
    ...
    def noteonoff(self, val, note=60, channel=None, velocity=None, transpose=0, **kwargs):
        note += transpose
        ...
        if val:
            ...
            self.midiout.send(MidiEvent.fromdata(NOTE_ON, channel=channel,
            ...
            self.midiout.send(MidiEvent.fromdata(NOTE_OFF, channel=channel,
                                                 data=[note & 0x7f, velocity & 0x7f]))
    ...
    def mute_channel(self, value, channel=None, invert=False, **kwargs):
        ...
        val = 0 if value >= 0.5 else self._controllers[channel].get(CHANNEL_VOLUME, 127)

        self.midiout.send(MidiEvent.fromdata(CONTROLLER_CHANGE, channel=channel,
                                             data=[CHANNEL_VOLUME, val]))
    ...
    def solo_channel(self, value, channel=None, invert=False, **kwargs):
        ...
        for ch in range(16):
            ...
            val = 0 if value >= 0.5 else self._controllers[ch].get(CHANNEL_VOLUME, 127)

            self.midiout.send(MidiEvent.fromdata(CONTROLLER_CHANGE, channel=ch,
                                                 data=[CHANNEL_VOLUME, val]))

    def sendpc(self, value, program=0, bank=None, bank_msb=None, bank_lsb=None, channel=None,
               **kwargs):
        if not value:
            return
        ...
        if bank_msb is not None:
            self.midiout.send(MidiEvent.fromdata(CONTROLLER_CHANGE, channel=channel,
                                                 data=[BANK_SELECT_MSB, bank_msb]))

        if bank_lsb is not None:
            self.midiout.send(MidiEvent.fromdata(CONTROLLER_CHANGE, channel=channel,
                                                 data=[BANK_SELECT_LSB, bank_msb]))

        if (bank_msb, bank_lsb) != (None, None):
            self._bank[channel] = (bank_lsb or 0) * 128 + (bank_msb or 0)

        self._program[channel] = program
        self.midiout.send(MidiEvent.fromdata(PROGRAM_CHANGE, channel=channel, data=[program]))
    ...
    def sendpcrel(self, value, channel=None, **kwargs):
        ...
        self.midiout.send(MidiEvent.fromdata(PROGRAM_CHANGE, channel=channel,
    ...
    def sendpb(self, value, channel=None, **kwargs):
        ...
        pb = int(2**14 * value)
        self.midiout.send(MidiEvent.fromdata(PITCH_BEND, channel=channel,
    ...
    def sendmp(self, value, channel=None, **kwargs):
        ...
        self.midiout.send(MidiEvent.fromdata(CHANNEL_PRESSURE, channel=channel, data=[value]))
    ...
    def sendpp(self, value, note=0, channel=None, transpose=0, **kwargs):
        ...
        self.midiout.send(MidiEvent.fromdata(POLYPHONIC_PRESSURE, channel=channel,
                                             data=[(note + transpose) & 0x7f, value]))
...
class OSC2MIDIServer(liblo.ServerThread):
    def __init__(self, dispatcher, rport=5555, shost=None, sport=9000):
...
def main(args=None):
    ...
    aadd('--version', action='version', version=__version__)
    ...
    server = OSC2MIDIServer(dispatcher, args.osc_rport, args.osc_shost, args.osc_sport)
