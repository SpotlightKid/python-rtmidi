...
pkg_resources.require("TurboGears>=1.1b1dev-r4343,<1.9")
...
def init_database(args):
    ...
    try:
        u = session.query(model.User).filter_by(user_name=u'admin').one()
    except model.InvalidRequestError:
        u = model.User(display_name=u"Administrator", user_name=u"admin",
            password=u"admin", email_address=u"admin@localhost.localdomain")
        g = model.Group(display_name=u"Package maintainers",
            group_name=u"maintainers")
        u.groups.append(g)
        p = model.Permission(permission_name=u"upload",
